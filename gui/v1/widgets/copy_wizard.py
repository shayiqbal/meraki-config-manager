"""Copy Rules wizard — 5-step guided workflow for multi-network rule copying."""
...
from services.copy_service import CopyResult, CopyService, NetworkCopyPreview
...
class CopyWizardPanel(QWidget):
    """5-step wizard: source → rules → destinations → preview → results."""

    def __init__(self, app: "MainWindowV1", parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self._app = app
        self._networks: list[dict[str, Any]] = []
        self._source_rules: list[VpnExclusionRule] = []
        self._selected_rules: list[VpnExclusionRule] = []
        self._previews: list[NetworkCopyPreview] = []
        self._results: list[CopyResult] = []
    ...
    def _build_source(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lay.setSpacing(10)
        lay.addWidget(_step_header(1, 5, "Select Source Network",
                                   "Choose the network whose Local Breakout rules you want to copy."))
    ...
    def _build_rules(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lay.setSpacing(10)
        lay.addWidget(_step_header(2, 5, "Select Rules to Copy",
                                   "Check the exclusion rules you want to copy to other networks."))
        sel_row = QHBoxLayout()
        sel_all = QPushButton("Select All")
        sel_all.clicked.connect(lambda: self._check_all_rules(True))
        clr = QPushButton("Clear")
        clr.clicked.connect(lambda: self._check_all_rules(False))
        ...
        self._rules_table.setHorizontalHeaderLabels(["✓", "Type", "Protocol", "Destination / App", "Port"])
        self._rules_table.setAlternatingRowColors(True)
        self._rules_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.Stretch)
        ...
        lay.addWidget(self._rules_table)

        self._rules_status = QLabel("")
        self._rules_status.setObjectName("section-sub")
        lay.addWidget(self._rules_status)
        return w
    ...
    def _build_dests(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lay.setSpacing(10)
        lay.addWidget(_step_header(3, 5, "Select Destination Networks",
                                   "Choose one or more networks to receive the selected rules. "
    ...
    def _build_preview(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lay.setSpacing(10)
        lay.addWidget(_step_header(4, 5, "Preview Changes",
                                   "Review every rule that will be added or skipped per network. "
    ...
    def _build_results(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lay.setSpacing(10)
        lay.addWidget(_step_header(5, 5, "Results",
                                   "Deployment complete. Rules are listed as Added or Already Existed per network."))
    ...
    def _load_source_rules(self, network: dict[str, Any]) -> None:
        client = self._app.get_client()
        org_id = self._app.get_org_id()
        if not client or not org_id:
            return
        self._source_status.setText(f"Loading rules for {network['name']}…")
        self._app.run_worker(
            lambda: client.get_rules(org_id, network["id"]),
            on_result=self._on_source_rules,
            status_msg=f"Loading rules for {network['name']}…",
        )

    def _on_source_rules(self, ruleset) -> None:
        self._source_rules = ruleset.rules
        self._populate_rules_table()
        net = self._selected_source()
        self._source_status.setText(
            f"{len(self._source_rules)} rule(s) in {net['name'] if net else '—'}"
        )
    ...
    def _populate_rules_table(self) -> None:
        ...
        for rule in sorted(self._source_rules, key=lambda r: r.order):
            row = self._rules_table.rowCount()
            self._rules_table.insertRow(row)
            chk = QCheckBox()
            chk.setChecked(True)
            self._rules_table.setCellWidget(row, 0, chk)
            self._rules_table.setItem(row, 1, QTableWidgetItem(rule.category.value))
            self._rules_table.setItem(row, 2, QTableWidgetItem(rule.protocol))
            self._rules_table.setItem(
                row, 3, QTableWidgetItem(rule.destination or rule.name or "")
            )
            self._rules_table.setItem(row, 4, QTableWidgetItem(rule.port))
            self._rules_table.item(row, 1).setData(Qt.ItemDataRole.UserRole, rule)

    def _check_all_rules(self, checked: bool) -> None:
        for row in range(self._rules_table.rowCount()):
            w = self._rules_table.cellWidget(row, 0)
            if isinstance(w, QCheckBox):
                w.setChecked(checked)

    def _collect_selected_rules(self) -> None:
        self._selected_rules = []
        for row in range(self._rules_table.rowCount()):
            w = self._rules_table.cellWidget(row, 0)
            if isinstance(w, QCheckBox) and w.isChecked():
                rule = self._rules_table.item(row, 1).data(Qt.ItemDataRole.UserRole)
                if rule:
                    self._selected_rules.append(rule)
        self._rules_status.setText(f"{len(self._selected_rules)} rule(s) selected")
    ...
    def _check_all_dests(self, checked: bool) -> None:
        ...
        for i in range(self._dests_list.count()):
            item = self._dests_list.item(i)
            if not item.isHidden():  # only affect visible (non-filtered) items
    ...
    def _generate_preview(self) -> None:
        ...
        self._apply_btn.setEnabled(False)
        copy_svc = self._app.get_copy_service()
        self._app.run_worker(
            lambda: copy_svc.preview(org_id, rules, dests),
            on_result=self._on_preview,
            status_msg="Generating preview…",
        )

    def _gate_apply(self) -> None:
        ok = self._confirm_edit.text().strip().upper() == "CONFIRM" and bool(self._previews)
        self._apply_btn.setEnabled(ok)

    def _on_preview(self, previews: list[NetworkCopyPreview]) -> None:
        self._previews = previews
        lines: list[str] = []
        for p in previews:
            lines.append(f"▶  {p.network_name}")
            lines.append(f"   Changes: {p.new_count} new rule(s) will be added  |  "
                         f"{p.exists_count} already exist (skipped)  |  "
                         f"{p.invalid_count} invalid")
            lines.append("")
            for s in p.rule_statuses:
                badge = {"new": "[NEW]", "exists": "[EXISTS]", "invalid": "[INVALID]"}[s.status]
                dest = s.rule.destination or s.rule.name or s.rule.application_id or "—"
                lines.append(
                    f"   {badge:10}  {s.rule.category.value:20}  "
                    f"{s.rule.protocol:6}  {dest:40}  port:{s.rule.port}"
                )
            lines.append("")
        ...
        self._apply_btn.setEnabled(False)
        if total_new == 0:
            self._apply_btn.setText("No New Rules to Apply")
        else:
            self._apply_btn.setText(f"✓ Deploy {total_new} Rule(s) to {len(previews)} Network(s)")
    ...
    def _apply(self) -> None:
        ...
        answer = QMessageBox.question(
            self,
            "Confirm Copy",
            f"Copy {len(self._selected_rules)} rule(s) to {len(dests)} network(s)?\n\n"
            f"{net_names}\n\n"
            "Rules that already exist on a destination will be skipped automatically.",
        )
        ...
        def _do() -> list[CopyResult]:
            return copy_svc.execute(org_id, rules, dests)

        def _done(results: list[CopyResult]) -> None:
            self._progress.setValue(100)
            self._results = results
            self._show_results()
            self._goto(4)
            self._app.log_activity(
                f"Copy complete: {sum(r.rules_added for r in results)} rule(s) added across "
                f"{sum(r.success for r in results)}/{len(results)} network(s).",
                "SUCCESS" if all(r.success for r in results) else "WARNING",
            )

        self._app.run_worker(_do, on_result=_done, status_msg="Copying rules…")

    def _show_results(self) -> None:
        lines: list[str] = []
        total_added  = sum(r.rules_added  for r in self._results)
        total_skipped = sum(r.rules_skipped for r in self._results)
        ok_nets = sum(r.success for r in self._results)
        total_nets = len(self._results)

        lines.append(
            f"Deployment complete: {total_added} rule(s) added, "
            f"{total_skipped} already existed, across {ok_nets}/{total_nets} network(s)"
        )
        lines.append("")

        # Build a lookup: network_id → NetworkCopyPreview
        ...
        for res in self._results:
            ...
            if res.success:
                lines.append(
                    f"   {res.rules_added} rule(s) added  |  "
                    f"{res.rules_skipped} already existed (skipped)"
                )
                preview = preview_by_id.get(res.network_id)
                if preview:
                    added   = [s for s in preview.rule_statuses if s.status == "new"]
                    existed = [s for s in preview.rule_statuses if s.status == "exists"]
                    invalid = [s for s in preview.rule_statuses if s.status == "invalid"]
                    ...
                    if added:
                        ...
                        for s in added:
                            dest = s.rule.destination or s.rule.name or s.rule.application_id or "—"
                            lines.append(
                                f"   + [{s.rule.category.value:20}]  "
                                f"{s.rule.protocol:6}  {dest}"
                            )
                    ...
                    if invalid:
                        ...
                        for s in invalid:
                            dest = s.rule.destination or s.rule.name or s.rule.application_id or "—"
                            lines.append(f"   ! [{s.rule.category.value}]  {dest}  — {s.detail}")
            else:
    ...
    def _build_export_rows(self) -> list[dict[str, str]]:
        """Build a flat list of rows (one per rule per network) for export."""
        ...
        for net_id, preview in preview_by_id.items():
            ...
            deploy_status = "Success" if (res and res.success) else ("Failed" if res else "Unknown")
            for s in preview.rule_statuses:
                rule = s.rule
                dest = rule.destination or rule.name or rule.application_id or "—"
                rows.append({
                    "Network":        preview.network_name,
                    "Deploy Status":  deploy_status,
                    "Rule Status":    "Added" if s.status == "new"
                                      else "Already Existed" if s.status == "exists"
                                      else "Invalid",
                    "Rule Type":      rule.category.value,
                    "Protocol":       rule.protocol,
                    "Destination / Application": dest,
                    "Port":           rule.port,
                    "Detail":         s.detail,
                })
        return rows
    ...
    def _export_csv(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            ...
            f"copy_rules_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
            "CSV files (*.csv)",
        )
        if not path:
            return
        ...
        self._app.log_activity(f"Copy results exported to CSV: {Path(path).name}", "SUCCESS")

    def _export_excel(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Excel Report",
            f"copy_rules_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
            "Excel files (*.xlsx)",
        )
        if not path:
            return
        ...
        for row in rows:
            ...
            r = ws.max_row
            status = row.get("Rule Status", "")
            fill = (added_fill if status == "Added"
        ...
        wb.save(path)
        self._app.log_activity(f"Copy results exported to Excel: {Path(path).name}", "SUCCESS")
